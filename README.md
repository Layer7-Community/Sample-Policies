...
|[**Portal Policy Template Bundles**](./Portal-Templates)|A collection of importable policy template bundles for the Layer7 API Developer Portal. Each bundle installs an encapsulated assertion plus its backing policy fragment under `/Portal Templates/` to provide Portal-aware authentication (API key, API key + secret, OAuth 2.0, mTLS), routing, and drop-down selection-field examples. See the folder's [README](./Portal-Templates/README.md) and the [Layer7 API Developer Portal Policy Templates Guide](./Portal-Templates/Layer7%20API%20Developer%20Portal%20Policy%20Templates%20Guide.md) for details.|
